...
class Block(Widget):
    def __init__(self,name,nameID,inputConnector,outputConnector,nParams,nUsers,nSwitches,tapSwitch, **kwargs):
        ...
        self.name = name
        self.ID = nameID
        self.Xpos = random.randrange(200, 800)
        self.Ypos = random.randrange(200, 600)
    ...
    def move_block(self,touch):
        if self.selected == SELECTED:
            if touch.pos[X] > 20:
                if touch.pos[X] + self.rect.size[X] < SCREEN_WIDTH * 2: #SCREEN_WIDTH
                    if touch.pos[Y] > 20:
                        if touch.pos[Y] + self.rect.size[Y] < SCREEN_HEIGHT * 2: #SCREEN_HEIGHT
